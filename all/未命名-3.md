...
sequenceDiagram
    ...
    C->>S: ClientHello
    Note left of C: 1. 生成随机数 CR<br/> 2. 支持的 TLS 版本（最高为 1.3）<br/> 3. 支持的密码套件列表（1.3 格式）<br/> 4. 扩展（SNI、ALPN、key_share、psk 等）<br/> 5. 生成一个临时私钥、临时公钥
    Note right of C: key_share 扩展：<br/> 客户端直接发送临时公钥 A
    Note over C,S: 关键变化：<br/> 客户端提前猜测密钥交换参数，<br/> 在第一条消息就发送公钥，<br/> 旨在实现 1-RTT 甚至 0-RTT

    S->>C: ServerHello
    Note left of S: 返回：<br/> 1. 选定的 TLS 版本（1.3）<br/> 2. server_random（SR）<br/> 3. 选定的密码套件<br/> 4. key_share 扩展（服务器临时公钥 B）
